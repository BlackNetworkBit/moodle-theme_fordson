 ...
 */
namespace theme_fordson\settings;
use admin_setting_configtext;

defined('MOODLE_INTERNAL') || die;

class fordson_admin_setting_configinteger extends admin_setting_configtext {
    ...
    public function validate($data) {
        if (!is_number($data)) {
            $validated = get_string('asconfigintnan', 'theme_fordson', array('value' => $data));
        } else {
            $validated = parent::validate($data); // Pass parent validation first.

            if ($validated == true) {
                if ($data < $this->lower) {
                    $validated = get_string('asconfigintlower', 'theme_fordson', array('value' => $data, 'lower' => $this->lower));
                } else if ($data > $this->upper) {
                    $validated = get_string('asconfigintupper', 'theme_fordson', array('value' => $data, 'upper' => $this->upper));
                } else {
                    $validated = true;
                }
            }
