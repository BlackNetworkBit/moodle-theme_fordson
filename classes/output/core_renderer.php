    ...
    */
    ...
    public function social_icons() {
        ...
        $socialcontext = [
            ...
            'hassocialnetworks' => ($hasfacebook || $hastwitter || $hasgoogleplus || $hasflickr || $hasinstagram || $hasvk || $haslinkedin || $haspinterest || $hasskype || $haslinkedin || $haswebsite || $hasyoutube || $hasblog || $hasvimeo || $hastumblr || $hassocial1 || $hassocial2 || $hassocial3) ? true : false, 'socialicons' => [
                [
                    'haslink' => $hasfacebook,
                    'linkicon' => 'facebook'
                ],
    ...
    public function fp_wonderbox() {
        global $PAGE;
        $context = $this->page->context;
        $hascreateicon = (empty($PAGE->theme->settings->createicon && isloggedin() && has_capability('moodle/course:create',
                $context))) ? false : $PAGE->theme->settings->createicon;
        ...
        $fp_wonderboxcontext = ['logintoken' => $logintoken, 'hasfptextbox' => (!empty($PAGE->theme->settings->fptextbox && isloggedin())), 'fptextbox' => $fptextbox, 'hasslidetextbox' => (!empty($PAGE->theme->settings->slidetextbox && isloggedin())), 'slidetextbox' => $slidetextbox, 'hasfptextboxlogout' => !isloggedin(), 'fptextboxlogout' => $fptextboxlogout, 'hasshowloginform' => $PAGE->theme->settings->showloginform, 'alertbox' => $alertbox, 'hasmarkettiles' => ($hasmarketing1 || $hasmarketing2 || $hasmarketing3 || $hasmarketing4 || $hasmarketing5 || $hasmarketing6) ? true : false, 'markettiles' => [
            ...
            // If any of the above social networks are true, sets this to true.
            'hasfpiconnav' => ($hasnav1icon || $hasnav2icon || $hasnav3icon || $hasnav4icon || $hasnav5icon || $hasnav6icon || $hasnav7icon || $hasnav8icon || $hascreateicon || $hasslideicon) ? true : false, 'fpiconnav' => [
                [
                    'hasicon' => $hasnav1icon,
    ...
    public function customlogin() {
        ...
        $customlogin_context = ['hascustomlogin' => $hascustomlogin, 'hasdefaultlogin' => $hasdefaultlogin, 'hasfeature1' => !empty($PAGE->theme->setting_file_url('feature1image',
                'feature1image')) && !empty($PAGE->theme->settings->feature1text), 'hasfeature2' => !empty($PAGE->theme->setting_file_url('feature2image',
                'feature2image')) && !empty($PAGE->theme->settings->feature2text), 'hasfeature3' => !empty($PAGE->theme->setting_file_url('feature3image',
                'feature3image')) && !empty($PAGE->theme->settings->feature3text), 'feature1image' => $PAGE->theme->setting_file_url('feature1image',
            'feature1image'), 'feature2image' => $PAGE->theme->setting_file_url('feature2image',
            'feature2image'), 'feature3image' => $PAGE->theme->setting_file_url('feature3image',
            'feature3image'), 'feature1text' => (empty($PAGE->theme->settings->feature1text)) ? false : format_text($PAGE->theme->settings->feature1text,
            FORMAT_HTML,
            [
                'noclean' => true
            ...
            // If any of the above social networks are true, sets this to true.
            'hasfpiconnav' => ($hasloginnav1icon || $hasloginnav2icon || $hasloginnav3icon || $hasloginnav4icon) ? true : false, 'fpiconnav' => [
                [
    ...
    public function fp_marketingtiles() {
        ...
        $fp_marketingtiles = ['hasmarkettiles' => ($hasmarketing1 || $hasmarketing2 || $hasmarketing3 || $hasmarketing4 || $hasmarketing5 || $hasmarketing6) ? true : false, 'markettiles' => [
            [
                'hastile' => $hasmarketing1,
                'tileimage' => $marketing1image,
                'content' => $marketing1content,
    ...
    public function fp_slideshow() {
        ...
        $fp_slideshow = ['hasfpslideshow' => $slideshowon, 'hasslide1' => $hasslide1 ? true : false, 'hasslide2' => $hasslide2 ? true : false, 'hasslide3' => $hasslide3 ? true : false, 'showtext1' => $showtext1 ? true : false, 'showtext2' => $showtext2 ? true : false, 'showtext3' => $showtext3 ? true : false, 'slide1' => [
            'slidetitle' => $slide1,
    ...
    public function teacherdashmenu() {
        ...
        $haspermission = has_capability('enrol/category:config',
                $context) && $PAGE->theme->settings->coursemanagementtoggle && isset($COURSE->id) && $COURSE->id > 1;
        $togglebutton = '';
        $togglebuttonstudent = '';
        $hasteacherdash = '';
        $hasstudentdash = '';
    ...
    public function teacherdash() {
        ...
        $participantstitle = ($PAGE->theme->settings->studentdashboardtextbox == 1) ? false : get_string('participants',
            'moodle');
        ...
        // Student Dash
        if (progress::get_course_progress_percentage($PAGE->course)) {
            $comppc = progress::get_course_progress_percentage($PAGE->course);
        ...
        $showonlygroupteachers = !empty(groups_get_all_groups($course->id,
                $USER->id)) && $PAGE->theme->settings->showonlygroupteachers == 1;
        ...
        if ($role) {
            ...
            foreach ($teachers as $staff) {
                ...
                $picture = $OUTPUT->user_picture($staff,
                    [
                        'size' => 50
                    ]);
    ...
    public function display_ilearn_secure_alert() {
        ...
        if ($cm && $quiz && $globalhasilearnsecureplugin) {
            $quiz_record = $DB->get_record('quizaccess_ilearnbrowser',
                [
                    'quiz_id' => $quiz->id
                ]);
